...
static void Scan_Mode() {
    while (radar.mode == Scan) {
        if (radar.angle > radar.settings[Max_Angle].Value)
            radar.angle = radar.settings[Max_Angle].Value;
        if (radar.angle < radar.settings[Min_Angle].Value)
            radar.angle = radar.settings[Min_Angle].Value;
...
static void Drow_Radius_line(uint8_t angle, uint8_t radius, HAL_SSD1306_Color color) {
    uint8_t x = angle > 90 ? RADAR_CENTER - floor(cos(DEG_TO_RAD(angle)) * radius)
...
static void handle_button_click() {
    ...
    if (HAL_GPIO_ReadPin(radar.Button_Port, radar.Button_Pin) == GPIO_PIN_HIGH) {
        ...
        if (__HAL_SCR1_TIMER_GET_TIME() - radar.button_press_time >
            HOLD_DELAY_MS * SYSTEM_FREQ_KHZ) {
